...
\begin{frame}{Exemplo de Probabilidade Condicional vs Probabilidade Conjunta}
    \centering
        \begin{tikzpicture}
            \begin{axis}
            \addplot3[samples=50,surf,domain=-4:4,domain y=-4:4,shader=interp] 
                {1/(2 *pi* sqrt(1-0.8^2))* exp(-(x^2+y^2-2*0.8*x*y)/(2*(1-0.8^2)))};
...
\begin{frame}{Exemplo de Probabilidade Condicional vs Probabilidade Conjunta}
    \begin{tikzpicture}
        ...
        \addplot3[samples=50,contour,domain=-4:-4,domain y=-4:-4]
            {1/(2 *pi* sqrt(1-0.8^2))* exp(-(x^2+y^2-2*0.8*x*y)/(2*(1-0.8^2)))};
        \draw[red,line width=2pt] (-4,1.5) -- (4,1.5);
        \end{axis}

        \begin{axis}
        \addplot[samples=50,red,line width=2pt,domain=-4:-4]
            {1/(2 *pi* sqrt(1-0.8^2))* exp(-(x^2+1.5^2-2*0.8*x*y)/(2*(1-0.8^2)))};
        \end{axis}
    \end{tikzpicture}
\end{frame}

% (Discrete) Moon Stuff here: https://github.com/betanalpha/stan_intro/blob/master/1-logic.tex
% (Continuous) Distance here: https://github.com/betanalpha/stan_intro/blob/master/1-logic.tex
