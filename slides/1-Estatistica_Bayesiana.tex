...
\begin{frame}{Teorema de Bayes como Motor de Inferência}
    ...
    $$
    \underbrace{P(\theta \mid y)}_{Posterior} = \frac{\overbrace{P(y \mid  \theta)}^{\text{Verossimilhança}} \cdot \overbrace{P(\theta)}^{\textit{Priori}}}{\underbrace{P(y)}_{\text{Constante Normalizadora}}}
    $$
    \begin{vfilleditems}
