...
\begin{frame}{Um interlúdio de Integrador Numéricos}
  ...
  em um futuro tempo $t$ a partir de condições inciais específicas.
\end{frame}
...
\begin{frame}{Limitações do Algorito HMC}
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      de meio-passos $L$ e do tamanho do passo $\epsilon$. Em especial o integrador
...
\begin{frame}{\textbf{N}o-\textbf{U}-\textbf{T}urn-\textbf{S}ampler (NUTS)}
  ...
  \vfill
  Isto quer dizer que tal algoritmo não \textbf{permitira meia-voltas} (\textit{u-turns}).
\end{frame}
...
\begin{frame}{Algoritmo de NUTS}
    ...
    $\boldsymbol{\theta}^{(*)} \leftarrow \boldsymbol{\theta}$\;
    \Para{$1, 2, \dots, 2L$}{
     Escolha uma direção $v \sim \text{Uniforme}\left( \left\{-1, 1 \right\} \right)$\;
     ...
     \eSe{
       A distância a diferença entre os vetores
       ...
       \textcolor{red}{Pare a amostragem de $\boldsymbol{\theta}^{(*)}$ na direção $v$ e continue apenas amostrando na direção $-v$}\;
        ...
        \Se{A distância a diferença entre os vetores
...
\begin{frame}{Correntes Markov Convergentes}
  \begin{figure}
    \centering
...
\begin{frame}[fragile]{Mensagens de Erro do \href{https://mc-stan.org}{\texttt{Stan}}}
...
\begin{frame}{Correntes Markov Divergentes}
  \begin{figure}
    \centering
