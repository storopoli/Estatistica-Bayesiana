...
$$
\log(\boldsymbol{y})= \alpha + \beta_1 x_1 + \beta_2 x_2 + \dots + \beta_k x_k
$$

que é o mesmo que:

$$
\boldsymbol{y} = e^{(\alpha + \beta_1 x_1 + \beta_2 x_2 + \dots + \beta_k x_k)}
$$
...
\boldsymbol{y} &= e^{\alpha } \cdot e^{\mathbf{X} \boldsymbol{\beta}}
\end{aligned}
$$
...
-   `treatment`: se o apartamento foi dedetizado há uma **redução** de 60% de baratas exterminada
-   `senior`: se o apartamento possui somente idosos há uma redução de 70% de barata exterminadas
