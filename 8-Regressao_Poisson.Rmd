...
Podemos fazer uma regressão de Poisson se $y$ for uma variável com dados de contagem, ou seja, $y$ somente toma valores positivos. A função de verossimilhança de Poisson usa uma constante $\alpha$ e os coeficientes $\boldsymbol{\beta}$ porém estes são exponenciados $e^x$:
...
Vamos chegar o *Posterior Predictive Check* do modelo binomial na figura \@ref(fig:pp-check-poisson):
