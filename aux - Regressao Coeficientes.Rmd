...
* **Coluna padronizada `Standardized`**: a cada 1 desvio padrão de $x$ de incremento (dp = `r sd(df$x)`), há um aumento de `r model$standardized.coefficients[2]` desvio padrão de $y$ (`r model$standardized.coefficients[2] * sd(df$y)`). Um total de `r model$standardized.coefficients[2] * sd(df$y) * model$coefficients[2]`.
