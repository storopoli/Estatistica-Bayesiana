...
Já na verossimilhança binomial modelamos uma variável dependente contínua $y$ que é o número de successos de $n$ experimentos Bernoulli independentes.
...
p &\sim \text{Logística}(\alpha + X \cdot \boldsymbol{\beta}) \\
...
* $y$ -- variável dependente contínua - successos de $n$ experimentos Bernoulli independentes
...
Stan instanciará um modelo bayesiano com os dados fornecidos ($y$ e $X$) e encontrará a distribuição posterior dos parâmetros de interesse do modelo ($\alpha$ e $\boldsymbol{\beta}$) calculando a distribuição posterior completa de:
...
O modelo possuirá as seguintes variáveis como independentes: `dist`,`arsenic`, `assoc`, `educ`. Para todas essas variáveis, como estão em uma escala logit (que é o inverso da transformação logística), usarei uma *priori* de uma distribuição normal com média 0 e desvio padrão 2.5. O que resulta em `prior = normal(0, 2.5)`. Vou colocar a *priori* da constante $\alpha$ como uma normal centrada na média da variável dependente `switch` e com um desvio padrão 2.5 maior que o desvio padrão de `switch`, basicamente traduzindo o que usamos como *priori* em modelos gaussianos (regressão linear). Isto resulta em `prior_intercept = normal(0, 2.5 * sd(wells$switch))`. Notem que aqui não temos erro do modelo, pois a função de verossimilhança não faz pressupostos sobre desvios como era o caso na regressão linear com uma verossimilhança Gaussiana/Normal. Portanto não há o que se especificar para `prior_aux`.
...
model_binomial <- stan_glm(
  ...
  family = binomial,
  prior = normal(0, 2.5),
  prior_intercept = normal(0, 2.5 * sd(wells$switch))
)
```
...
Vamos chegar o *Posterior Predictive Check* do modelo binomial na figura \@ref(fig:pp-check-binomial):
...
$$
\begin{aligned}
p &\sim \text{Logística}(\alpha + X \cdot \beta) \\
p &\sim \text{Logística}(\alpha) + \text{Logística}(X \cdot \boldsymbol{\beta}) \\
...
Fazemos icco om a função `exp()` do `R` nos coeficientes do `modelo_binomial`:
