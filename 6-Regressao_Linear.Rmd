...
* $\alpha$ uma constante (também chamada de *intercept*)
* $\boldsymbol{\beta}$ um vetor de coeficientes
* $\textbf{X}$ uma matriz de dados
* $\epsilon$ o erro do modelo.
...
Stan instanciará esse modelo bayesiano com os dados fornecidos ($y$ e $X$) e encontrará a distribuição posterior dos parâmetros de interesse do modelo ($\alpha$ e $\boldsymbol{\beta}$) calculando a distribuição posterior completa de:
...
Ao todo serão 4 modelos para modelar QI da criança (`kid_score`). Os primeiros dois modelos terão apenas um único preditor (`mom_hs` ou `mom_iq`), o terceiro usará dois preditores (`mom_hs + mom_iq`) e o quarto incluirá uma interação entre esses dois preditores (`mom_hs * mom_iq`).
...
Segundo modelo é apenas a variável `mom_iq` como preditora. Vou manter as *prioris* de $\alpha$ e $\lambda$. Para a variável `mom_iq` acredito que a correlação entre QI da mãe e QI da criança seja positiva e acima de 0.5. Então a minha *priori* será uma normal centrada em 0.5 com 0.5 de desvio padrão. Isto resulta em `prior = normal(0.5, 0.5)`:
...
Terceiro modelo usa as duas variáveis `mom_hs` e `mom_iq` como preditoras. Vou manter todas as *prioris* definidas nos modelos 1 e 2.
...
Quarto modelo usa as duas variáveis `mom_hs` e `mom_iq` como preditoras por meio de uma interação entre as duas. Vou manter as mesmas *prioris* dos modelos anteriores. Note que a formula agora é atualizada para:
