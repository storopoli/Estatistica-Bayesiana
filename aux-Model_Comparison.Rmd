...
$$
\operatorname{elpd}_{\text{loo}} = sum_i^N \log p(y_i \mid y_{-i})
$$
...
Stan e suas interfaces (`rstanarm` e `brms`) conseguem calcular $\operatorname{elpd}$ com o pacote [`loo`](https://mc-stan.org/loo) [@loo]. Conseguimos calcular LOO, WAIC e $K$-fold CV com as funções:
...
Na [Aula 8 - Regressão de Poisson](8-Regressao_Poisson.html) tínhamos 3 modelos que estimamos:

* Regressão de Poisson usando o `rstanarm`
* Regressão binomial usando o `rstanarm`
...
Então calculamos todos os LOOs dos modelos:
...
Conseguimos comparar os modelos agora de maneira objetiva com o `loo_compare(criterion = c("loo", "kfold", "waic")` usando um dos critérios `"loo"`, `"waic"` ou `"kfold"`. Por robustez e facilidade de computação eu usarei o `"loo"`:

```{r loo_compare}
loo::loo_compare(loo_poisson, loo_negbinomial, loo_zero_inflated)
```

`elpd_diff` é a diferença de $\operatorname{elpd}_{\text{loo}}$ para os modelos relativo ao que tem o maior valor de $\operatorname{elpd}_{\text{loo}}$. Ou seja, o modelo com maior $\operatorname{elpd}_{\text{loo}}$ recebe o valor de 0 na comparação.
